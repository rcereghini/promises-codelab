...
const app = (() => {
		...
		const promiseOfImageName = new Promise((resolve, reject) => {
			setTimeout(() => {
				if (country === "spain" || country === "chile" || country === "peru") {
					...
					reject(Error("Didn'nt receive a valid country name!"));
				}
			}, 1000);
		});
		...
		const promiseOfImageName = new Promise((resolve, reject) => {
			setTimeout(() => {
				if (country === "spain") {
					...
					reject(Error("Received value other than Spain."));
				}
			}, 3000);
		});
		console.log("promiseOfImageName", promiseOfImageName);
		return promiseOfImageName;
	}
	...
	var promises = [getImageName("Spain222"), getImageName("Chile"), getImageName("Peru")];
